...
class InternalController {
    ...
    @GetMapping("/users/{id}/followers") FollowerPage followers(@RequestHeader("X-Internal-Key") String key, @PathVariable("id") UUID id, @RequestParam(value = "cursor", required = false) String cursor, @RequestParam(value = "limit", defaultValue = "100") int limit) { check(key); return social.followers(id, cursor, Math.min(limit, 100)); }
    @GetMapping("/users/{id}/following/celebrities") List<UUID> celebrities(@RequestHeader("X-Internal-Key") String key, @PathVariable("id") UUID id) { check(key); return social.followedCelebrities(id); }
