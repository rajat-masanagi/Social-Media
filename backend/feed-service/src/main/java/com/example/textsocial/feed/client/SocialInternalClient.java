...
public interface SocialInternalClient {
    @GetMapping("/internal/users/{id}/followers") FollowerPage followers(@PathVariable("id") UUID id, @RequestParam("cursor") String cursor, @RequestParam("limit") int limit);
    @GetMapping("/internal/users/{id}/following/celebrities") List<UUID> celebrities(@PathVariable("id") UUID id);
